...
const LoginPage = () => {
  ...
  const { loading, error, success, userData, login } = useAuth()

  const onFinish = async (values) => {
    console.log('🔵 LoginPage.onFinish викликано', values)
    try {
      console.log('📤 Викликаємо функцію login')
      await login({
        email: values.email,
        password: values.password
      })
      console.log('✅ login успішно завершено')
    } catch (err) {
      console.error('❌ Помилка в onFinish', err)
