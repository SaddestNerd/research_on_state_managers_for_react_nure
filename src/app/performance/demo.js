...
class PerformanceDemo {
	...
	async step1_InitialState() {
		...
		console.log(`📊 Кількість вимірювань дій: ${report.actions.stats.count}`)
	...
	async step4_StressTest() {
		...
		console.log(
			`   Дій за секунду: ${stressResults.actionsPerSecond.toFixed(0)}`
		)
		...
		console.log(
			`  - Пам\'ять: ${
				preview.memory.latest
					? (preview.memory.latest.usedJSHeapSize / 1024 / 1024)
					: 'N/A'
			} МБ`
		)
