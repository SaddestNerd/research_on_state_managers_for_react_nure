...
class PerformanceTester {
	...
	async simulateActions(count = 100, rootStore = null) {
		...
		for (let i = 0; i < count; i++) {
			...
			if (i % 2 === 0) {
				store.authStore.clearError()
			} else {
				store.productsStore.clearError()
			}
	...
	async memoryTest(rootStore = null) {
		...
		const measureData = metrics.startActionMeasure('memoryTest/loadProducts')
		runInAction(() => {
			store.productsStore.products = largeArray
			store.productsStore.pagination.total = largeArray.length
		})
		metrics.endActionMeasure(measureData)
	...
	async runFullTest(rootStore = null) {
		console.log('\n═══════════════════════════════════════════════════════════')
		console.log('🔬 ЗАПУСК ПОВНОГО ТЕСТУ ПРОДУКТИВНОСТІ (MobX)')
		console.log('═══════════════════════════════════════════════════════════\n')
		...
		if (!rootStore) {
			...
			metrics.setStoreInitTime(initTime)
			console.log(`🚀 Час ініціалізації MobX stores: ${initTime.toFixed(3)} мс`)
		}

		await this.simulateActions(50, rootStore)

		const stressResults = await this.stressTest(500)

		const memoryResults = await this.memoryTest(rootStore)
	...
	async selectorBenchmark(count = 1000, rootStore = null) {
		console.log(`\n🎯 Бенчмарк доступу до MobX stores: ${count} циклів...`)
		const store = rootStore || new RootStore()
		const start = performance.now()
		for (let i = 0; i < count; i++) {
			metrics.measureSelector('benchmark.selectAuth', () => ({
				userData: store.authStore.userData,
				isAuthenticated: store.authStore.isAuthenticated,
			}))
			metrics.measureSelector('benchmark.selectProducts', () => ({
				products: store.productsStore.products,
				pagination: store.productsStore.pagination,
			}))
		}
	...
	exportResults(filename = 'mobx-performance-results.json') {
...
Додаткові команди для тестування:
  ...
  🔹 window.selectorBenchmark(2000)
     Бенчмарк доступу до MobX stores (виконати багаторазові виклики)
